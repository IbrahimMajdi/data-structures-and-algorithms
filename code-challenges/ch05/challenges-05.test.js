...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(item => {
    let clo = $('#template').clone();
    clo.find('h2').text(item.name);
    clo.find('h3').text(item.height);
    clo.find('p').text(item.eye_color);
    $('main').append(clo);
  })
...
describe('Testing challenge 1', () => {
  test('It should append the star wars people to the DOM', () => {
    ...
    expect($('section:nth-child(4) p').text()).toStrictEqual('yellow');
  })
});
